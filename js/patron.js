...
const colRecords =
  _SYS_CFG && _SYS_CFG.cols && _SYS_CFG.cols.rec
    ? _SYS_CFG.cols.rec
    : "trio_records";
const colNotifs =
  _SYS_CFG && _SYS_CFG.cols && _SYS_CFG.cols.ntf
    ? _SYS_CFG.cols.ntf
    : "trio_notifications";
...
async function loginBoss() {
  ...
  if (!db) {
    alert(
      "SİSTEM HATASI: Veri tabanı bağlantısı kurulamadı. Lütfen internet bağlantınızı kontrol edip sayfayı yenileyin.",
    );
    return;
  }
  ...
  try {
    ...
    if (maintDoc.exists && maintDoc.data().enabled === true) {
      return alert(
        `🔧 ${maintDoc.data().message || "Sistem bakım modundadır."}`,
      );
    }
  } catch (e) {
...
function renderServiceDist(counts) {
  ...
  sorted.slice(0, 10).forEach(([srv, count]) => {
    // Show top 10 now since we have space
    const percent = (
      (count / sorted.reduce((a, b) => a + b[1], 0)) *
      100
    ).toFixed(0);
    list.innerHTML += `
        <div class="srv-row">
...
function loadRecentActivity() {
  ...
  if (!db) {
    list.innerHTML =
      '<div style="color:red; font-size:12px;">Bağlantı Hatası</div>';
    return;
  }
  ...
  db.collection(colNotifs)
    ...
    .then((snap) => {
      ...
      if (snap.empty)
        list.innerHTML =
          '<div style="opacity:0.5; padding:20px; text-align:center">Aktivite yok.</div>';
    })
    .catch((error) => {
      console.error("Activity Load Error:", error);
      list.innerHTML =
        '<div style="color:red; font-size:12px;">Veri yükleme hatası.</div>';
    });
}
