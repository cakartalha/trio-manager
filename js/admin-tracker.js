...
async function getIPAddress() {
  try {
    // 3 saniye timeout ile yarışı başlat (Mobil şebekelerde takılmayı önler)
    const fetchPromise = fetch(_SYS_CFG.net.ep);
    ...
    console.warn("IP fetch cancelled (Using fallback):", error.message);
    return "Bilinmiyor (Mobil/VPN)";
  }
}
...
async function startTrackingSession(panel, userId = null) {
  ...
  try {
    ...
    const sessionRef = await db
      .collection(_SYS_CFG.cols.adm_ses)
      .add(sessionData);
...
async function endTrackingSession() {
  ...
  try {
    ...
    await db.collection(_SYS_CFG.cols.adm_ses).doc(sessionId).update({
      isActive: false,
...
function startRemoteCommandListener() {
  ...
  const unsubscribe = db
    .collection(_SYS_CFG.cols.rem_cmd)
      ...
      snapshot.docChanges().forEach((change) => {
        if (change.type === "added") {
          ...
          const commandTime = command.timestamp
            ? command.timestamp.toDate().getTime()
            : 0;
          const now = new Date().getTime();

          if (now - commandTime < 60000) {
            // 1 dakikadan yeniyse uygula
...
function hasExecuted(cmdId) {
  const executed = JSON.parse(
    sessionStorage.getItem("trio_executed_cmds") || "[]",
  );
  return executed.includes(cmdId);
}

function markExecuted(cmdId) {
  let executed = JSON.parse(
    sessionStorage.getItem("trio_executed_cmds") || "[]",
  );
  executed.push(cmdId);
